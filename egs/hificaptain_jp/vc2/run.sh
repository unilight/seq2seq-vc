...
db_root=downloads/hi-fi-captain/ja-JP
dumpdir=dump                # directory to dump full features
srcspk=male                  # available speakers: "clb" "bdl"
trgspk=female                  # available speakers: "slt" "rms"
stats_ext=h5
...
if [ -z ${norm_name} ]; then
    ...
    src_stats="${dumpdir}/${srcspk}_train_parallel/stats.${stats_ext}"
    trg_stats="${dumpdir}/${trgspk}_train_parallel/stats.${stats_ext}"
else
    if [ -z ${pretrained_model_checkpoint} ]; then
...
if [ "${stage}" -le 1 ] && [ "${stop_stage}" -ge 1 ]; then
    ...
    pids=()
    for name in "${srcspk}_train_parallel" "${srcspk}_dev" "${srcspk}_eval" "${trgspk}_train_parallel" "${trgspk}_dev" "${trgspk}_eval"; do
    (
...
if [ "${stage}" -le 2 ] && [ "${stop_stage}" -ge 2 ]; then
    ...
    # src
    spk="${srcspk}"
    for name in "${spk}_train_parallel" "${spk}_dev" "${spk}_eval"; do
    (
...
if [ "${stage}" -le 3 ] && [ "${stop_stage}" -ge 3 ]; then
    ...
    if [ ! -z ${pretrained_model_checkpoint} ]; then
        ...
        cp "${dumpdir}/${trgspk}_train_parallel/stats.${stats_ext}" "${expdir}/"
        echo "Training start. See the progress via ${expdir}/train.log."
        ${cuda_cmd} --gpu "${n_gpus}" "${expdir}/train.log" \
            vc_train.py \
                --config "${conf}" \
                --src-train-dumpdir "${dumpdir}/${srcspk}_train_parallel/norm_${norm_name}" \
                --src-dev-dumpdir "${dumpdir}/${srcspk}_dev/norm_${norm_name}" \
                --src-feat-type "${src_feat}" \
                --trg-train-dumpdir "${dumpdir}/${trgspk}_train_parallel/norm_${norm_name}" \
                --trg-dev-dumpdir "${dumpdir}/${trgspk}_dev/norm_${norm_name}" \
                --trg-feat-type "${trg_feat}" \
                --trg-stats "${expdir}/stats.${stats_ext}" \
                --train-dp-input-dir "${dumpdir}/${srcspk}_train_parallel/norm_${norm_name}" \
