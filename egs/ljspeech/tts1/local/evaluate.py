...
def _calculate_mcd_f0(file_list, gt_root, f0min, f0max, results):
    for i, cvt_wav_path in enumerate(file_list):
        ...
        gt_wav, gt_fs = librosa.load(gt_wav_path, sr=cvt_fs)
        assert cvt_fs == gt_fs
...
def main():
    ...
    gt_root = os.path.join(args.data_root, "wavs")
    transcription_path = os.path.join(args.data_root, "metadata.csv")
    ...
    with mp.Manager() as manager:
        ...
        for f in file_lists:
            p = mp.Process(
                ...
                args=(f, gt_root, f0min, f0max, results),
            )
            p.start()
            processes.append(p)
