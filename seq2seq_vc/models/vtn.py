...
class VTN(torch.nn.Module):
    ...
    def __init__(
        ...
        self.encoder.embed[-1].alpha.data = torch.tensor(1.0)
        self.decoder.embed[-1].alpha.data = torch.tensor(1.0)

    def forward(self, xs, ilens, ys, labels, olens, spembs=None, *args, **kwargs):
    # def forward(self, xs, ilens, spembs=None, *args, **kwargs):
        max_ilen = max(ilens)
        max_olen = max(olens)
        ...
        # Modify mask. Use ceiling division here
        ilens_st = ilens.new(
            [torch.div(torch.div(ilen - 2 + 1, 2, rounding_mode='floor') - 2 + 1, 2, rounding_mode='floor') for ilen in ilens]
        )
