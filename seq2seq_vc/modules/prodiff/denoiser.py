...
class SpectogramDenoiser(nn.Module):
    ...
    def inference(self, condition: torch.Tensor) -> torch.Tensor:
        ...
        coef1 = (beta * torch.sqrt(alph_prod_prv) / (1.0 - alph_prod)).to(device)
        coef2 = ((1.0 - alph_prod_prv) * torch.sqrt(alph) / (1.0 - alph_prod)).to(
            device
        )
        post_var = beta * (1.0 - alph_prod_prv) / (1.0 - alph_prod)
        post_var = torch.log(torch.maximum(post_var, torch.full((1,), 1e-20))).to(
            device
        )
